...
import { FlaskConical, ScanLine, LogOut, CheckCircle2, Clock, XCircle, AlertTriangle, Search, Menu, X } from 'lucide-react';
...
export default function LabTechDashboard({ user, onLogout }: { user: any; onLogout: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row">
      {/* Mobile Header */}
      <header className="md:hidden flex items-center justify-between p-4 bg-slate-900 text-white sticky top-0 z-[60]">
        <div className="flex items-center gap-3">
          <Menu className="w-6 h-6" onClick={() => setIsSidebarOpen(true)} />
          <h1 className="text-sm font-black uppercase tracking-wider">Lab Portal</h1>
        </div>
        <div className="w-8 h-8 bg-indigo-600 rounded-full flex items-center justify-center text-[10px] font-black">
          {user?.name?.[0]}
        </div>
      </header>
      ...
      {/* Sidebar */}
      <aside className={`
        fixed md:relative inset-y-0 left-0 w-64 bg-slate-900 text-white p-6 flex flex-col z-[80] transition-transform duration-300
      ...
      `}>
        <div className="flex items-center justify-between mb-10">
          ...
          <X className="md:hidden w-6 h-6 text-slate-400" onClick={() => setIsSidebarOpen(false)} />
        </div>
        <nav className="space-y-2 flex-1">
          <button onClick={() => { setActiveTab('labs'); setIsSidebarOpen(false); }}
          ...
          <button onClick={() => { setActiveTab('scans'); setIsSidebarOpen(false); }}
      ...
      <main className="flex-1 p-4 md:p-8 overflow-x-hidden">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
          <div>
            <h2 className="text-xl md:text-2xl font-black">{activeTab === 'labs' ? 'Laboratory Requests' : 'Scan / Imaging Requests'}</h2>
            <p className="text-slate-500 text-xs md:text-sm">Review and enter diagnostic results</p>
          </div>
          <div className="flex bg-slate-100 p-1 rounded-xl gap-1 self-start lg:self-center overflow-x-auto max-w-full">
            {['pending', 'processing', 'completed', 'all'].map(f => (
              <button key={f} onClick={() => setFilter(f)}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all capitalize whitespace-nowrap ${filter === f ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}>
                {f}
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <div className="bg-white p-4 rounded-2xl border border-slate-200"><p className="text-[9px] md:text-xs text-slate-400 font-bold uppercase">Pending</p>
            <p className="text-xl md:text-2xl font-black text-amber-600">{activeTab === 'labs' ? pendingLabs : pendingScans}</p></div>
          <div className="bg-white p-4 rounded-2xl border border-slate-200"><p className="text-[9px] md:text-xs text-slate-400 font-bold uppercase">Processing</p>
            <p className="text-xl md:text-2xl font-black text-blue-600">{(activeTab === 'labs' ? labs : scans).filter(i => i.status === 'processing' || i.status === 'scheduled').length}</p></div>
          <div className="bg-white p-4 rounded-2xl border border-slate-200"><p className="text-[9px] md:text-xs text-slate-400 font-bold uppercase">Completed</p>
            <p className="text-xl md:text-2xl font-black text-green-600">{(activeTab === 'labs' ? labs : scans).filter(i => i.status === 'completed').length}</p></div>
          <div className="bg-white p-4 rounded-2xl border border-slate-200"><p className="text-[9px] md:text-xs text-slate-400 font-bold uppercase">Total</p>
            <p className="text-xl md:text-2xl font-black">{activeTab === 'labs' ? labs.length : scans.length}</p></div>
        </div>
      ...
      <AnimatePresence>
        {selectedItem && (
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4 md:p-6">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white w-full max-w-md rounded-[32px] shadow-2xl p-6 md:p-8 max-h-[90vh] overflow-y-auto">
