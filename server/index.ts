...
app.post('/api/appointments', async (req, res) => {
  ...
  try {
    ...
    await sendSMS(phoneNumber, `PRIME: Your appointment ${appointmentId} is booked for ${preferredDate} at ${preferredTime}. Location: ${locationLink}. Thank you for choosing Primecare.`);
  ...
  try {
    ...
    if (apt) {
      if (status === 'approved') {
        ...
        const locationLink = "https://www.google.com/maps/search/?api=1&query=Primecare+Medical+Center+Accra";
        const msg = `PRIME: Your appointment ${apt.appointment_id} has been APPROVED with ${doctorName} for ${dateStr}. Location: ${locationLink}. Call +233200024081 for enquiries. Thank you for choosing Primecare.`;
        ...
        const msg = `PRIME: Your appointment ${apt.appointment_id} with ${doctorName} has been marked as COMPLETED. Call +233200024081 for enquiries. Thank you for choosing Primecare.`;
        await sendSMS(apt.phone_number, msg).catch(e => console.error('SMS Error in Edit/Complete:', e));
      } else if (status === 'cancelled') {
        const msg = `PRIME: Your appointment ${apt.appointment_id} has been CANCELLED. Call +233200024081 for enquiries. Thank you for choosing Primecare.`;
